"""
Multi-Agent Travel Planning System with Chainlit UI
...
# Try to import chainlit, make it optional for testing
try:
    import chainlit as cl
    CHAINLIT_AVAILABLE = True
except ImportError:
    CHAINLIT_AVAILABLE = False
    print("⚠️ Chainlit not installed. Running in standalone mode.")
    print("To install Chainlit: pip install chainlit")  
...
# Chainlit decorators (only if Chainlit is available)
if CHAINLIT_AVAILABLE:
    @cl.on_chat_start
    async def start():
        """Initialize the app when chat starts"""
        global app
        try:
            ...
            await cl.Message(
                content="🧳 **Welcome to the Multi-Agent Travel Planning System!**\n\n"
                       "I'm your AI travel assistant with specialized agents for:\n"
                       "✈️ **Flight Booking** - Find and compare flights\n"
                       "🏨 **Hotel Search** - Discover accommodations\n"
                       "🗺️ **Itinerary Planning** - Create detailed travel plans\n\n"
                       "Just tell me what you need help with, and I'll route you to the right specialist!"
            ).send()
        except Exception as e:
            await cl.Message(
                content=f"❌ **Error initializing the travel planner:**\n{str(e)}\n\n"
    ...
    async def main(message: cl.Message):
        ...
        if app is None:
            await cl.Message(
                content="❌ **Travel planner not initialized.** Please refresh the page and try again."
            ).send()
            return
        ...
        try:
            # Get session ID for consistent thread management
            session_id = cl.user_session.get("session_id", "default")
            
            # Process the message with consistent session ID
            response = app.chat(message.content, thread_id=session_id)
            ...
            thinking_msg.content = f"❌ **Error processing your request:**\n{str(e)}"
            await thinking_msg.update()


# For standalone testing
def test_main():
    """Test function for running without Chainlit"""
    print("🧳 Multi-Agent Travel Planning System - Test Mode")
    print("=" * 50)
...
if __name__ == "__main__":
    ...
    import sys
    if CHAINLIT_AVAILABLE and 'chainlit' in sys.modules:
        # Running with Chainlit - the decorators will handle everything
        pass
    else:
